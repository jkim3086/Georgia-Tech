...
pfn_t pagefault_handler(vpn_t request_vpn, int write) {
   ...
   */
   ...
   if(victim_pcb != NULL) {
	   ...
	   pte_t *target = &(victim_pcb->pagetable[victim_vpn]);
	   
	   if(target->dirty) {
			page_from_disk(victim_pfn, victim_vpn, victim_pcb->pid);
	   }
  ...
  /* FIX ME */
	pte_t *update = &(current_pagetable[request_vpn]);
	
	update->pfn = victim_pfn;
	update->dirty = write;
	update->used = 1;
	update->valid = 1;
