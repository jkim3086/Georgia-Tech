...
double compute_emat() {
    ...
    */
	...
	long int Hits = MEMORY_ACCESS_TIME * count_tlbhits;
	long int Misses = MEMORY_ACCESS_TIME * tlb_misses;
	long int Page_Faults = DISK_ACCESS_TIME * count_pagefaults;
	long int NUM = Hits + Misses + Page_Faults;

   return (NUM / total_accesses);
}
